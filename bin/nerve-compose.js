...
function parseCliOptions(argv) {
  ...
  if (!subcommand) {
    throw new Error('Usage: nerve-compose <modules|doctor|validate|add|init> [workspaceDir] [--json] [--builtin-only] [--strict]; add docs <minimal|ai> [workspaceDir]; add mcp [workspaceDir] [--inline]; add host <composable> [workspaceDir]')
  }
...
function scaffoldMemorySetupHelper(workspaceDir) {
  const helperPath = path.join(workspaceDir, 'db-helpers', 'memory-setup.js')
  return upsertGeneratedMemoryHelperFile(
    helperPath,
    GENERATED_MEMORY_SETUP_HELPER_MARKER,
    buildGeneratedMemorySetupHelperSource(),
    'db-helpers/memory-setup.js already exists and is not generated by nerve-compose memory scaffold',
  )
}

function scaffoldMemoryHealthHelper(workspaceDir) {
  const helperPath = path.join(workspaceDir, 'db-helpers', 'memory-health.js')
  return upsertGeneratedMemoryHelperFile(
    helperPath,
    GENERATED_MEMORY_HEALTH_HELPER_MARKER,
    buildGeneratedMemoryHealthHelperSource(),
    'db-helpers/memory-health.js already exists and is not generated by nerve-compose memory scaffold',
  )
}
...
async function runAddCommand(options) {
  ...
  if (options.capability === 'memory-pgvector') {
    ...
    const serverRelativePath = './mcp-servers/local-mcp.mjs'
