...
test('nerve-compose add memory-pgvector scaffolds workspace files', async () => {
  ...
  try {
    ...
    const memorySetupSource = await readFile(path.join(workspaceRoot, 'db-helpers', 'memory-setup.js'), 'utf8')
    ...
    const memoryHealthSource = await readFile(path.join(workspaceRoot, 'db-helpers', 'memory-health.js'), 'utf8')
...
test('nerve-compose add memory-pgvector skips manual merge for non-generated db helper files', async () => {
  ...
  await writeFile(path.join(workspaceRoot, 'nextv.json'), JSON.stringify({ entrypointPath: 'entry.nrv' }, null, 2), 'utf8')
  await mkdir(path.join(workspaceRoot, 'db-helpers'), { recursive: true })
  await writeFile(path.join(workspaceRoot, 'db-helpers', 'memory-setup.js'), 'export const manual = true\n', 'utf8')
  ...
  try {
    ...
    const payload = JSON.parse(result.stdout)
    assert.equal(payload.ok, false)
    const setupEntry = payload.files.find((entry) => String(entry.path || '').replace(/\\/g, '/').endsWith('db-helpers/memory-setup.js'))
    assert.equal(Boolean(setupEntry), true)
    assert.equal(setupEntry.action, 'skipped_manual_merge')

    const preserved = await readFile(path.join(workspaceRoot, 'db-helpers', 'memory-setup.js'), 'utf8')
...
test('nerve-compose add mcp scaffolds local server and external module config by default', async () => {
  ...
  try {
    ...
    const serverSource = await readFile(path.join(workspaceRoot, 'mcp-servers', 'local-mcp.mjs'), 'utf8')
    ...
    const mcpConfigRaw = await readFile(path.join(workspaceRoot, 'mcp.json'), 'utf8')
    ...
    assert.equal(nextv.modules?.mcp?.configPath, './mcp.json')
  } finally {
    await rm(workspaceRoot, { recursive: true, force: true })
  }
})
...
test('nerve-compose add mcp --inline keeps inline module config', async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => readFile(path.join(workspaceRoot, 'mcp.json'), 'utf8'),
      /ENOENT/,
    )
