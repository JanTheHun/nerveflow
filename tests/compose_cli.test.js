...
test('nerve-compose add host composable scaffolds local host files', async () => {
  ...
  try {
    ...
    const serverPath = path.join(workspaceRoot, 'host', 'server.js')
...
test('nerve-compose add host defaults to composable profile when omitted', async () => {
  ...
  try {
    ...
    assert.equal(payload.ok, true)
    assert.equal(payload.capability, 'host:composable')
    assert.equal(existsSync(path.join(workspaceRoot, 'host', 'server.js')), true)
  } finally {
    await rm(workspaceRoot, { recursive: true, force: true })
  }
})
...
test('nerve-compose add host returns manual merge warning for user-managed host files', async () => {
  ...
  await writeFile(path.join(workspaceRoot, 'host', 'server.js'), 'console.log("custom host")\n', 'utf8')
  ...
  try {
    ...
    const payload = JSON.parse(result.stdout)
    assert.equal(payload.ok, false)
    const blockedServer = payload.files.find((entry) => String(entry.path || '').replace(/\\/g, '/').endsWith('host/server.js'))
    assert.equal(Boolean(blockedServer), true)
    assert.equal(blockedServer.action, 'skipped_manual_merge')

    const preservedServer = await readFile(path.join(workspaceRoot, 'host', 'server.js'), 'utf8')
...
test('nerve-compose add mcp scaffolds local server and module config', async () => {
  ...
  try {
    const result = await runProcess(['bin/nerve-compose.js', 'add', 'mcp', 'local-tools', workspaceRelativePath, '--json'])
    assert.equal(result.code, 0)

    const payload = JSON.parse(result.stdout)
    assert.equal(payload.ok, true)
    assert.equal(payload.command, 'add')
    assert.equal(payload.capability, 'mcp:local-tools')
    assert.equal(payload.composable?.providerLabel, 'mcp')
    assert.equal(payload.composable?.host, 'composable-reference-host')
    assert.equal(payload.composable?.autoAttach, true)

    const serverSource = await readFile(path.join(workspaceRoot, 'mcp-servers', 'local-tools.mjs'), 'utf8')
    ...
    assert.deepEqual(nextv.modules?.mcp?.servers?.[0], {
      ...
      config: {
        command: 'node',
        args: ['./mcp-servers/local-tools.mjs'],
      },
    })
  } finally {
    await rm(workspaceRoot, { recursive: true, force: true })
...
test('nerve-compose add mcp is idempotent on rerun', async () => {
  ...
  try {
    ...
    const payload = JSON.parse(second.stdout)
    assert.equal(payload.ok, true)
    assert.equal(payload.capability, 'mcp:local-mcp')
    const actions = payload.files.map((entry) => entry.action)
    assert.equal(actions.every((action) => action === 'unchanged' || action === 'updated' || action === 'skipped'), true)
  } finally {
    await rm(workspaceRoot, { recursive: true, force: true })
