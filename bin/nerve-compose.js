...
function parseCliOptions(argv) {
  ...
  if (!subcommand) {
    throw new Error('Usage: nerve-compose <modules|doctor|validate|add|init> [workspaceDir] [--json] [--builtin-only] [--strict]; add docs <minimal|ai> [workspaceDir]; add mcp [serverName] [workspaceDir]')
  }
  ...
  for (let i = 0; i < rest.length; i += 1) {
    ...
    if (options.subcommand === 'add' && options.capability === 'mcp' && !options.addName && !options.workspaceDir) {
      ...
      const looksLikeWorkspace = token === '.'
        || token === '..'
        || token.includes('/')
        || token.includes('\\')
        || /^[A-Za-z]:/.test(token)
...
async function runAddCommand(options) {
  ...
  const payload = buildAddPayload({
    capability: options.capability === 'transport' || options.capability === 'model' || options.capability === 'mcp'
